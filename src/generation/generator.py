...
2a. Если вопрос касается конкретного факта, имени, числа или названия — найди его в контексте и процитируй ДОСЛОВНО, символ в символ, как он написан в контексте. НЕ перефразируй и не пересказывай своими словами имена собственные, названия и числа — только точная копия из текста. Если ты не можешь найти точную дословную формулировку в контексте — это признак того, что ответа в контексте нет (см. правило 3), а не повод придумать похожую формулировку.
...
5. ОБЯЗАТЕЛЬНО начинай ответ с полноценного предложения, отвечающего на вопрос по существу. Строка "Источники: ..." всегда идёт ПОСЛЕ содержательного ответа, никогда не в начале и никогда не вместо него.
6. Отвечай кратко и по существу, без лишних рассуждений вслух.
7. Если контекст противоречив (разные фрагменты говорят разное) — укажи это в ответе, а не выбирай произвольно одну версию.
"""
...
def generate_answer(question: str, chunks: list[dict], model: str = "qwen2.5", temperature: float = 0.1) -> dict:
    ...
    Защита от "пустого" ответа: если модель вопреки правилам пропустила
    содержательную часть и начала ответ сразу с "Источники:" (наблюдалось
    на практике у некоторых локальных моделей при перегруженном промпте),
    делаем одну повторную попытку генерации — это дешевле и надёжнее, чем
    пытаться бесконечно "довести до идеала" сам промпт.
    """
    if not chunks:
        return {
    ...
    if answer_text.strip().lower().startswith("источники"):
        # Ответ пропущен — пробуем ещё раз (одна повторная попытка)
        answer_text = call_ollama(SYSTEM_PROMPT, user_prompt, model=model, temperature=temperature)
